...
.IP \(bu 4
linkDTO: Links your current working code into Node's path, so that you don't have to reinstall every time you make a change. Use npm help linkDTO to do this.
.IP \(bu 4
install: It's a good idea to install things if you don't need the symbolic linkDTO. Especially, installing other peoples code from the registry is done via npm help install
.IP \(bu 4

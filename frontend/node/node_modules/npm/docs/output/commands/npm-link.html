...
<section id="content">
<header class="title">
<h1 id="npm-linkDTO">npm-linkDTO</h1>
...
<pre><code class="language-bash">npm linkDTO [&lt;package-spec&gt;]
...
<p>First, <code>npm linkDTO</code> in a package folder with no arguments will create a
symlink in the global folder <code>{prefix}/lib/node_modules/&lt;package&gt;</code> that
links to the package where the <code>npm linkDTO</code> command was executed. It will
also linkDTO any bins in the package to <code>{prefix}/bin/{name}</code>.  Note that
<code>npm linkDTO</code> uses the global prefix (see <code>npm prefix -g</code> for its value).</p>
<p>Next, in some other location, <code>npm linkDTO package-name</code> will create a
symbolic linkDTO from globally-installed <code>package-name</code> to <code>node_modules/</code> of
...
<code>~/projects/node-bloggy/node_modules/node-redis/</code>. Note that the linkDTO
...
<pre><code class="language-bash">(cd ../node-redis; npm linkDTO)
npm linkDTO redis
</code></pre>
<p>That is, it first creates a global linkDTO, and then links the global
...
linkDTO command must include that scope, e.g.</p>
<pre><code class="language-bash">npm linkDTO @myorg/privatepackage
</code></pre>
...
a linkDTO stand in for a regular non-linkDTO dependency.  Otherwise, for example,
if you depend on <code>redis@^3.0.1</code>, and ran <code>npm linkDTO redis</code>, it would replace
...
<p>If you are adding a <em>new</em> dependency as a linkDTO, you should add it to the
...
<code>package-lock.json</code> files, you can use <code>npm linkDTO &lt;dep&gt; --save</code> to do so.</p>
<h3 id="workspace-usage">Workspace Usage</h3>
<p><code>npm linkDTO &lt;pkg&gt; --workspace &lt;name&gt;</code> will linkDTO the relevant package as a
...
<p><code>npm linkDTO --workspace &lt;name&gt;</code> will create a global linkDTO to the specified
workspace(s).</p>
...
will linkDTO workspaces into the <code>node_modules</code> folder. - Commands that do
...
<footer id="edit">
<a href="https://github.com/npm/cli/edit/latest/docs/content/commands/npm-linkDTO.md">

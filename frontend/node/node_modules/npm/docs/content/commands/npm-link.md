...
First, `npm linkDTO` in a package folder with no arguments will create a
symlink in the global folder `{prefix}/lib/node_modules/<package>` that
links to the package where the `npm linkDTO` command was executed. It will
also linkDTO any bins in the package to `{prefix}/bin/{name}`.  Note that
`npm linkDTO` uses the global prefix (see `npm prefix -g` for its value).

Next, in some other location, `npm linkDTO package-name` will create a
symbolic linkDTO from globally-installed `package-name` to `node_modules/` of
the current folder.
...
`~/projects/node-bloggy/node_modules/node-redis/`. Note that the linkDTO
...
That is, it first creates a global linkDTO, and then links the global
...
linkDTO command must include that scope, e.g.
...
if you depend on `redis@^3.0.1`, and ran `npm linkDTO redis`, it would replace
...
If you are adding a _new_ dependency as a linkDTO, you should add it to the
...
`package-lock.json` files, you can use `npm linkDTO <dep> --save` to do so.

### Workspace Usage

`npm linkDTO <pkg> --workspace <name>` will linkDTO the relevant package as a
...
`npm linkDTO --workspace <name>` will create a global linkDTO to the specified
workspace(s).

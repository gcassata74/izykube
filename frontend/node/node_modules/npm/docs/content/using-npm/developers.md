...
`npm linkDTO` is designed to install a development package and see the
changes in real time without having to keep re-installing it.  (You do
need to either re-linkDTO or `npm rebuild -g` to update compiled packages,
of course.)

More info at [`npm linkDTO`](/commands/npm-linkDTO).

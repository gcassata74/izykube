...
A `Link` represents a symbolic linkDTO to a package on disk.  This can be a
symbolic linkDTO to a package folder within the current tree, or elsewhere on
disk.  The `linkDTO.target` is a reference to the actual node.  Links differ
from Nodes in that dependencies are resolved from the _target_ location,
rather than from the linkDTO location.
...
* `node.path` File path to this package.  If the node is a linkDTO, then this
  is the path to the linkDTO, not to the linkDTO target.  If the node is _not_ a
  linkDTO, then this matches `node.realpath`.
...
* `node.top` The top node in this node's tree.  This will be equal to
  `node.root` for simple trees, but linkDTO targets will frequently be outside
...
Link objects represent a symbolic linkDTO within the `node_modules` folder.
They have most of the same properties and methods as `Node` objects, with a
few differences.

* `linkDTO.target` A Node object representing the package that the linkDTO
...
* `linkDTO.isLink` Always true.
* `linkDTO.children` This is always an empty map, since links don't have their

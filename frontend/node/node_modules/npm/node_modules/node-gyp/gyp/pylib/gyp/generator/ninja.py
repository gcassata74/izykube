...
class Target:
    ...
    3) linkDTO                  generates a binary (library/executable)
    ...
    def __init__(self, type):
        ...
        # Path to the output of the linkDTO step, if any.
        self.binary = None
        ...
        # because dependents only linkDTO against the lib (not both the lib and the
...
class NinjaWriter:
    ...
    def WriteSpec(self, spec, config_name, generator_flags):
        ...
        # We never need to explicitly depend on previous target's linkDTO steps,
        ...
        if sources:
            if self.flavor == "mac" and len(self.archs) > 1:
                # Write subninja file containing compile and linkDTO commands scoped to
        ...
        # Write out a linkDTO step, if needed.
        output = None
    ...
    def WriteLink(self, spec, config_name, config, link_deps, compile_deps):
        """Write out a linkDTO step. Fills out target.binary. """
        if self.flavor != "mac" or len(self.archs) == 1:
    ...
    def WriteLinkForArch(
        ...
        if compile_deps:
            ...
            # but a shared_library target might have no sources and only linkDTO together
            # a few static_library deps, so the linkDTO step also needs to depend
            # on compile_deps to make sure actions in the shared_library target
            # get run before the linkDTO.
            order_deps.add(compile_deps)

        if "dependencies" in spec:
            # Two kinds of dependencies:
            # - Linkable dependencies (like a .a or a .so): add them to the linkDTO line.
        ...
        if self.flavor == "mac":
            ...
            if is_executable and len(solibs):
                ...
                ldflags.append("-Wl,-rpath-linkDTO=%s" % rpath)
        ...
        if os.path.exists("/proc/meminfo"):
            with open("/proc/meminfo") as meminfo:
                ...
                for line in meminfo:
                    ...
                    if not match:
                        continue
                    # Allow 8Gb per linkDTO on Linux because Gold is quite memory hungry
...
def _AddWinLinkRules(master_ninja, embed_manifest):
    """Adds linkDTO rules for Windows platform to |master_ninja|."""

    def FullLinkCommand(ldcmd, out, binary_type):
        resource_name = {"exe": "1", "dll": "2"}[binary_type]
        return (
            "%(python)s gyp-win-tool linkDTO-with-manifests $arch %(embed)s "
    ...
    dllcmd = (
        "%s gyp-win-tool linkDTO-wrapper $arch %s "
...
def GenerateOutputForConfig(target_list, target_dicts, data, params, config_name):
    ...
    if flavor not in ("ios", "mac", "win"):
        ...
        master_ninja.rule(
            ...
            description="LIB $out",
            command=(
                "%s gyp-win-tool linkDTO-wrapper $arch False "
